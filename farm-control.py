...
def get_grab_settings(target_server, bot_type, bot_index):
    if bot_type == 'main' and bot_index == 0: # Alpha Bot
        ...
        return target_server.get('auto_grab_enabled_sub', False), target_server.get('heart_threshold_sub', 50), {0: 0.7, 1: 1.7, 2: 2.4}
                ...
                try:
                    ...
                    for msg_item in messages:
                        if msg_item.get("author", {}).get("id") == yoru_bot_id and msg_item.get("embeds"):
                            ...
                            break
                except Exception as e: print(f"Lỗi đọc Yoru Bot: {e}", flush=True)
    ...
    def run_spam_cycle(task_id, channel_id, message, bots_to_use, inter_bot_delay):
        try:
            for bot in bots_to_use:
                try: bot.sendMessage(channel_id, message); time.sleep(inter_bot_delay)
                except Exception: pass
        finally:
            if task_id in spam_tasks_running: spam_tasks_running.remove(task_id)
        ...
        try:
            ...
            for group_name, group_data in groups.items():
                ...
                with bots_lock:
                    ...
                    bots_to_use = [sub_bots[i] for i in account_indices if i < len(sub_bots) and bot_active_states.get(f'sub_{i}', False)]
                
                if not bots_to_use: continue

                # Kiểm tra từng farm trong group
                for server in farms_in_group:
                    ...
                    if server.get('spam_message') and server.get('spam_channel_id'):
                        ...
                        task_id = f"spam_{server_id}"
                        if (now - last_spam) >= delay and task_id not in spam_tasks_running:
                            spam_tasks_running.add(task_id)
                            server['last_spam_time'] = now
                            threading.Thread(target=run_spam_cycle, args=(task_id, server['spam_channel_id'], server['spam_message'], bots_to_use, 2)).start()
            time.sleep(1)
        except Exception as e: print(f"[ERROR in spam_loop] {e}", flush=True); time.sleep(1)

# ... (Hàm reboot_bot, auto_reboot_loop, periodic_save_loop giữ nguyên logic cốt lõi nhưng cần cập nhật để xử lý main_bots và sub_bots)
...
<head>
    ...
    <title>Karuta Farm Control - Advanced</title>
...
<body>
    <div class="container">
        <div class="header"><h1 class="title">FARM CONTROL PANEL - ADVANCED</h1></div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function fetchStatus() { /* We can simplify by just reloading on most actions */ }
});
</script>
</body>
</html>
...
def api_main_panel_sync():
    ...
    for server in farm_servers:
        if server.get('group') in target_groups:
            ...
            server['auto_grab_enabled_sub'] = main_panel_settings['auto_grab_enabled_sub']
            server['heart_threshold_sub'] = main_panel_settings['heart_threshold_sub']
            # Sync Broadcast
...
def api_reboot_toggle_auto():
    ...
    if auto_reboot_enabled and (auto_reboot_thread is None or not auto_reboot_thread.is_alive()):
        auto_reboot_stop_event = threading.Event()
        auto_reboot_thread = threading.Thread(target=auto_reboot_loop, daemon=True).start()
